...
class ResNet(nn.Module):
    def __init__(self, block, num_block, width_mul=0.5, num_classes=58):
        ...
        self.avg_pool = nn.AvgPool2d(kernel_size=2, stride=2, padding=0) # 兼容海思fp16
        self.dropoutc5_x = nn.Dropout(p=0.2)  #
    ...
    def forward(self, x):
        ...
        # output = self.dropoutc5_x(output)
        output = self.conv5_x(output)
        # output = self.avg_pool(output)
        output = self.avg_pool(self.avg_pool(output))
