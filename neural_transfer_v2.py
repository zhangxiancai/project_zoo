...
class ContentandStyleLoss(nn.Module):
    ...
    def forward(self):
        ...
        for i in ls:
            cnn_c_img=self.cnn(img_load(self.c_img), out_layer=i).detach_()
            cnn_s_img = self.cnn(img_load(self.s_img), out_layer=i).detach_()
            cnn_g_img=self.cnn(self.base(), out_layer=i)
...
if __name__=='__main__':
    ...
    op = optim.SGD(loss_module.base.parameters(), lr=0.0001,momentum=0.9)
    ...
    for ep in range(5000):
        ...
        c_s_loss.backward()
        print(f'ep:{ep},loss:{c_s_loss},c_loss:{c_loss},s_loss:{s_loss},lr:{sched.get_last_lr()}')
        # step
        # with torch.no_grad():
        #     if ep%300==0:
        #         lr*=0.95**(ep//300)
        ...
        # debug
        # print(loss_module.cnn.conv1[0].weight[1,...])
        # print(loss_module.cnn.conv2_x[0].shortcut[0].weight[1, ...].reshape(1,32))

    # save
    res_img=loss_module.base.g_img.detach().numpy()
    ...
    # tensor([[[[5.8712e-01, 4.3734e-01, 3.1059e-01],
    #           [3.7022e-01, 2.8272e-01, 1.5620e-01],
    #           [2.9270e-01, 1.7042e-01, 8.0562e-02]],