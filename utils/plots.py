...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    # Plot image grid with labels 针对车牌检测
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            # per box
            for j, box in enumerate(boxes.T):
                plot_one_box(box, mosaic, color=None, line_thickness=tl)
            # draw landmarks
            landmarks=image_targets[:, 6:] #
            # per landmarks
            for lb in range(landmarks.shape[0]):
                for lj in range(4):
                    x,y = int(landmarks[lb,2*lj]*w)+block_x,int(landmarks[lb,2*lj+1]*h)+block_y
                    cv2.circle(mosaic,(x,y),radius=2*lj+2,color=[0,255,0],thickness=-1)
