...
class LoadFaceImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.stride = stride
        self.downscale=A.Downscale(scale_min=0.2,scale_max=0.8,p=0.2) # 用于检测模糊的车牌
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            # 应用downscale
            img=self.downscale(image=img)['image']
            # cv2.imwrite(f'/data1/xiancai/PLATE_DATA/other/test_05_07/0.1/{index}.jpg',img)
        ...
        if self.augment:
            ...
            if random.random() < hyp['fliplr']:
                ...
                if nL:
                    ...
                    lu = np.copy(labels[:, [5, 6]])
                    lb = np.copy(labels[:, [9, 10]])
                    labels[:, [5, 6]] = labels[:, [7, 8]]
                    labels[:, [7, 8]] = lu
...
def random_perspective(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            img = cv2.warpPerspective(img, M, dsize=(width, height), borderValue=(114, 114, 114)) # 1280 to 640
        else:  # affine
...
def box_candidates(box1, box2, wh_thr=2, ar_thr=100):  # box1(4,n), box2(4,n)
    # Compute candidate boxes: box1 before augment, box2 after augment, wh_thr (pixels), aspect_ratio_thr, area_ratio
    w1, h1 = box1[2] - box1[0], box1[3] - box1[1]
    w2, h2 = box2[2] - box2[0], box2[3] - box2[1]
    ar = np.maximum(w2 / (h2 + 1e-16), h2 / (w2 + 1e-16))  # aspect ratio
