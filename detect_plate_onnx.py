...
class inference:
    ...
    # img_size = (1088 * 2, 1920 * 2)
    img_size = (640,640)
    conf_thres = 0.35
    iou_thres = 0.5
    model_path='/home/xiancai/plate/yolov5_face_plate/Result/2022_03_31/v5sfp_best_dynamic.onnx'
    # model_path = '/home/xiancai/plate/yolov5_face_plate/Result/2022_01_07/plate_detect_01_07_640_640.onnx'
    ses = ort.InferenceSession(model_path)
    print(f'plate_detect model:{model_path}\nimg_size:{img_size}\nconf:{conf_thres}\niou:{iou_thres}')

    def detect_one_onnx(self, orgimg):
        '''
        车牌检测
        :param orgimg:
        :return det: n*[xyxy,0,landmarks0-9] tensor
        '''
        # load img
        orgimg = cv2.imread(orgimg) if isinstance(orgimg,str) else copy.deepcopy(orgimg)
        img0 = orgimg.copy()
        # resize origin image
        imgsz=self.img_size
        img = letterbox(img0, new_shape=imgsz)[0]
        # print(img.shape)
        # cv2.imwrite('/data1/xiancai/PLATE_DATA/other/0.jpg',img)
        ...
        # Inference
        t0 = time.time()
        outs=self.ses.run(['strid_8','strid_16','strid_32'],{'input':img})
        ...
        # Apply NMS
        pred = non_max_suppression_face(z, self.conf_thres, self.iou_thres)[0] # [0]=img0
...
if __name__ == '__main__':
    ...
    # input_path = '/data1/xiancai/PLATE_DATA/other/test/model_inference_result/'
    # sub_dir='_'.join(model_path.split('/')[-2:])+'-'+'_'.join(input_path.split('/')[-3:-1])+f'-imgsize{img_size}_conf{conf_thres}_iou{iou_thres}'
    # res_path=f'/data1/xiancai/PLATE_DATA/res_detect/{sub_dir}/'
