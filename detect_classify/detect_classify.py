...
def detect(img_address):
    ...
    # for item in out:
    if out.numel():
        box=scale_coords(img.shape[2:], out[:, :4], [H,W,3]).round() # Rescale coords (xyxy) from img1_shape to img0_shape
...
if __name__ =='__main__':
    ...
    #     #
    #     if pre is None:
    #         err.append(f'img:{i},pre:{pre},tru:{tru}')
    #         continue
    #     pre_id,tru_id=cls_ids[pre[0, -1]], cls_ids[tru]
    #     if pre_id!=tru_id:
    #         err.append(f'img:{i},pre:{pre},tru:{tru}')
    #     res[pre_id,tru_id]+=1
