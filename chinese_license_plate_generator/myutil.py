...
def generate_double_plate():
    '''
    双层黄牌 例：'皖A123B5' 或 '皖B123B学'
    :return:
    '''
    ...
    if random.random()<0.9:
        ...
        plat_numb = plate_number.generate_plate_number_yellow_gua()

    img = generator.generate_plate_special(plat_numb, 'yellow', True,True)

    img=cv2.resize(img,(94,48))
...
def run():
    for i in range(5000):
        img,plat_numb = generate_double_plate()
        cv2.imwrite(f'/data1/xiancai/PLATE_DATA/plate_double_generate/{i}_{plat_numb}.jpg',img)
        print(i)
