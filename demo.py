...
if __name__ == '__main__':
    ...
    cap = cv2.VideoCapture('/data1/xiancai/FACE_ANGLE_DATA/test/N25_01153854.111.mp4')
    ...
    with torch.no_grad():
        while (cap.isOpened()):
            ...
            from detect_util.detect_yolov5_face_onnx import detect_one_onnx
            faces=detect_one_onnx(frame).numpy()
            faces=[[face[:4],face[5:],face[4]] for face in faces]
            ...
            for box, landmarks, score in faces:
               ...
               # cv2.waitKey(5)
                ...
                rep_utils.plot_pose_cube(frame,  y_pred_deg, p_pred_deg, r_pred_deg, x_min + int(.5*(x_max-x_min)), y_min + int(.5*(y_max-y_min)), size = bbox_width)
